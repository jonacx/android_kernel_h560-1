...
#ifdef CONFIG_COMPAT
#define COMPAT_ICE40_IOCTL_CMD_META_CHIP_CHECK  0X11111111
#define COMPAT_ICE40_IOCTL_CMD_DOWNLOAD         0X22222222
#define COMPAT_ICE40_IOCTL_CMD_POWER_CTRL       _IOW(SPI_IOC_MAGIC, 10, __u32)
#define COMPAT_SPI_IOC_MESSAGE(N)               _IOW(SPI_IOC_MAGIC, 0, char[SPI_MSGSIZE(N)])
#define COMPAT_SPI_IOC_WR_MODE                  _IOW(SPI_IOC_MAGIC, 1, __u8)
#define COMPAT_SPI_IOC_WR_LSB_FIRST             _IOW(SPI_IOC_MAGIC, 2, __u8)
#define COMPAT_SPI_IOC_WR_BITS_PER_WORD         _IOW(SPI_IOC_MAGIC, 3, __u8)
#define COMPAT_SPI_IOC_WR_MAX_SPEED_HZ          _IOW(SPI_IOC_MAGIC, 4, __u32)
#define COMPAT_SPI_IOC_RD_MODE                  _IOR(SPI_IOC_MAGIC, 1, __u8)
#endif
...
#ifdef CONFIG_COMPAT
static long ice40_compat_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
     long err = 0;
	 ...
	 switch (cmd)
	 {
	    ...
	    case COMPAT_SPI_IOC_WR_MODE:
			break;
	    case COMPAT_SPI_IOC_WR_LSB_FIRST:
			break;
	    case COMPAT_SPI_IOC_WR_BITS_PER_WORD:
			break;
	    case COMPAT_SPI_IOC_WR_MAX_SPEED_HZ:
			break;
		case COMPAT_SPI_IOC_RD_MODE:
			break;
	    case COMPAT_SPI_IOC_MESSAGE(1):
			 if (arg32 == NULL)
			 {
				 err = -EINVAL;
				 break;    
			 ...
			 err = file->f_op->unlocked_ioctl(file, SPI_IOC_MESSAGE(1), arg32);
			 if (err){
				 LATTICE_SPIDEV_ERR("ICE40_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				 return err;
			 }
			 break;

		 case COMPAT_SPI_IOC_MESSAGE(2):
			 if (arg32 == NULL)
			 {
				 err = -EINVAL;
				 break;    
			 ...
			 err = file->f_op->unlocked_ioctl(file, SPI_IOC_MESSAGE(2), arg32);
			 if (err){
				 LATTICE_SPIDEV_ERR("ICE40_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				 return err;
			 }
			 break;
		 case COMPAT_ICE40_IOCTL_CMD_META_CHIP_CHECK:
			 if (arg32 == NULL)
			 {
				 err = -EINVAL;
				 break;    
			 ...
			 err = file->f_op->unlocked_ioctl(file, ICE40_IOCTL_CMD_META_CHIP_CHECK, arg32);
			 if (err){
				 LATTICE_SPIDEV_ERR("ICE40_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				 return err;
			 }
			 break;
		 case COMPAT_ICE40_IOCTL_CMD_DOWNLOAD:
			 if (arg32 == NULL)
			 {
				 err = -EINVAL;
				 break;    
			 ...
			 err = file->f_op->unlocked_ioctl(file, ICE40_IOCTL_CMD_DOWNLOAD, arg32);
			 if (err){
				 LATTICE_SPIDEV_ERR("ICE40_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				 return err;
			 }
			 break;
		 case COMPAT_ICE40_IOCTL_CMD_POWER_CTRL:
			 if (arg32 == NULL)
			 {
				 err = -EINVAL;
				 break;    
			 ...
			 err = file->f_op->unlocked_ioctl(file, ICE40_IOCTL_CMD_POWER_CTRL, arg32);
			 if (err){
				 LATTICE_SPIDEV_ERR("ICE40_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				 return err;
			 }
		 ...
		 default:
			 LATTICE_SPIDEV_ERR("unknown IOCTL: 0x%08x\n", cmd);
			// err = -ENOIOCTLCMD;
			 break;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = ice40_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
